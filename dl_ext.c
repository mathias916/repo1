...
egl[0].addr = sym_mod(droid_rt,"eglInitialize");nl_ 
egl[1].addr = sym_mod(droid_rt,"eglGetError"); nl_
egl[2].addr = sym_mod(droid_rt,"eglGetCurrentDisplay");nl_
egl[3].addr = 
sym_mod(droid_rt,"eglCreatePlatformWindowSurface");nl_

D1

int version[2] ={1,1};//major ,minor


castf(egl[0].addr)(GLContext, &version[0], &version[1]);

Display = castf__(egl[2].addr)();

egl[4].addr = 
sym_mod(droid_rt,"eglMakeCurrent");

castf(egl[4].addr)(Display, Surface, Window, GLContext);
