...
#define MOD(mod_v) ((mod_*)mod_v)
#define MODV(sz,mod_o) ((sz*)mod_o)

void * dl_load_mod(void * dl_load, void *mod)
{
char * err;
int eri;


((mod_*)mod)->info_t = malloc(sizeof(Dl_info));


eri =
dladdr( dlsym(dl_load,"dlsym")  ,((mod_*)mod)->info_t );

	
if(mod )                                                   ((mod_*)mod)->mod =  dl_load;
err = dlerror();
                                                           if(err){
        out_("%s",err);
        getc(stdin);                                       }
                                      
							   if(((mod_*)mod)->mod == NULL)                              out_("so load not found ");
  else{
  out_("so %s loaded ", eri ? 
MODV(Dl_info, MOD(mod)->info_t)->dli_fname : NULL);
...
void * mem_offset(void *addr_start, 
		  size_t width, size_t Lcount)
{
...
while ((pid = (int) (castf_(wait_call)(status)) ) != -1) ++frames;
if(frames)
